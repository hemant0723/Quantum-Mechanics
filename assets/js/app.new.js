...
class CourseApp {
  ...
  renderAttendance() {
    if (!this.studentSelect?.value) return;
    const selectedId = this.studentSelect.value;
  ...
  renderHomework() {
    ...
    const items = (this.DATA.homework || []).slice().sort((a, b) => (a.due || '').localeCompare(b.due || ''));
    box.innerHTML = items.length ? items.map(hw => {
      const showIframe = hw.link && /\.html?(#|\?|$)/i.test(hw.link);
      const iframe = showIframe ? `
        <div style="margin-top:.5rem">
          <iframe src="${hw.link}" title="${this.escapeHtml(hw.title)}" style="width:100%;height:75vh;border:1px solid var(--line);background:#fff" loading="lazy"></iframe>
        </div>
      ` : (hw.link ? `<div style="margin-top:.5rem"><a href="${hw.link}" class="link">Open</a></div>` : '');

      return `
        <div class="hw-card">
          <h3>${this.escapeHtml(hw.title)}</h3>
          ${hw.description ? `<p>${this.escapeHtml(hw.description)}</p>` : ''}
          <div class="meta"><span>Due: ${fmt(hw.due)}</span></div>
          ${iframe}
        </div>
      `;
    }).join('') : '<p class="muted">No homework assigned yet.</p>';
  }
  ...
  renderNotes() {
    ...
    list.innerHTML = notes.map(n => `
      <div class="hw-card">
        <div class="meta"><strong>${this.escapeHtml(n.title)}</strong><span class="muted">${this.escapeHtml(n.date)}</span></div>
        <div class="row-wrap">
    ...
    list.querySelectorAll('.open-note').forEach(btn => {
      btn.addEventListener('click', () => {
