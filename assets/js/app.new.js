...
class CourseApp {
  ...
  setupThemeToggle() {
    ...
    const apply = (mode) => {
      ...
      if (btn) {
        const isDark = (root.getAttribute('data-theme') || (window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light')) === 'dark';
  ...
  setupAnimations() {
    const toReveal = [
      ...document.querySelectorAll('.card, .table, .accordion details, section.panel, header .brand, header .header-actions, .tabs .tab')
    ];
  ...
  renderGradeProgress(selectedId, marksBy, opts = { progressId: 'grade-progress', textId: 'grade-progress-text', segmented: false }) {
    ...
    const hwMax = sum(hwList, a => a.max || a.maxMarks || 0);
    const hwGot = sum(hwList, a => (marksBy[a.id]?.marks));
  ...
  renderResults() {
    ...
    if (tbody) {
      ...
      const scaledEND = endMax ? (endGot / endMax) * TARGET_END : 0;
      tbody.innerHTML = [
        ['Homework (12)', hwGot, hwMax, Math.round(scaledHW*10)/10 + ' / ' + TARGET_HW],
  ...
  renderAttendanceCharts() {
    ...
    if (!selectedId || !progressWrap || !barsWrap) return;
  ...
  renderHomework() {
    ...
    // Animate expand/collapse and lazy-load iframes when a section toggles
    acc.querySelectorAll('details.hw-item').forEach(det => {
      det.addEventListener('toggle', () => {
        const content = det.querySelector('.hw-content');
        if (content) {
          if (det.open) {
